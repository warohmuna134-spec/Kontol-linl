<?php
// ============================================================
// MALZ BYPASS URL - XEMOZ API (FIXED)
// Support: SFL.gl, Linkvertise, YorURL, dll
// ============================================================
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['url'])) {
    ...
    if (filter_var($original_url, FILTER_VALIDATE_URL)) {
        // API Xemoz - bypasslink_izenlol
        $ch = curl_init();
        curl_setopt($ch, CURLOPT_URL, 'https://api-xemoz-official.my.id/api/tools/bypasslink_izenlol.php?url=' . urlencode($original_url));
        ...
        if ($response && $httpCode === 200) {
            ...
            // Extract result - format baru: result.destination_url
            if (isset($data['result']['destination_url']) && filter_var($data['result']['destination_url'], FILTER_VALIDATE_URL)) {
                $bypass_result = $data['result']['destination_url'];
            } elseif (isset($data['destination']) && filter_var($data['destination'], FILTER_VALIDATE_URL)) {
                $bypass_result = $data['destination'];
            } elseif (isset($data['url']) && filter_var($data['url'], FILTER_VALIDATE_URL)) {
                $bypass_result = $data['url'];
            } elseif (isset($data['result']) && is_string($data['result']) && filter_var($data['result'], FILTER_VALIDATE_URL)) {
                $bypass_result = $data['result'];
            } elseif (isset($data['bypass']) && filter_var($data['bypass'], FILTER_VALIDATE_URL)) {
                $bypass_result = $data['bypass'];
            } else {
                $error_message = 'Tidak dapat bypass URL. Sila cuba lagi.';
                $bypass_result = '';
            }
            
            // Kalau masih kosong, print response untuk debug
            if (empty($bypass_result)) {
                $error_message = 'API response: ' . htmlspecialchars(json_encode($data));
            }
        } else {
            $error_message = 'Server bypass sedang sibuk. Sila cuba sebentar lagi.';
        }
    } else {
